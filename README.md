...
The index.txt file (whose purpose is to implement MRU) is generated by cd'ing to $DOSROOT and running the following:
    ...
    find ./dosbox -type f -exec basename {} .conf \; > index.txt

And then I just use the following ~/.local/bin/dosgames script:
...
Along with the following kitty_fzf_preview script:

    #!/usr/bin/env bash
    kitten icat --clear --transfer-mode=memory --stdin=no --place="${FZF_PREVIEW_COLUMNS}x${FZF_PREVIEW_LINES}@0x0" --scale-up=yes "$@"
